...
- **Three more on-device effects — flip-o-rama, clock, and Game of Life.** They join plasma, fire
  ...
  - **`clock`** — a digital clock with big **HH:MM** in a bundled **anti-aliased** JetBrains Mono
    face (8-bit coverage, so the ink is scaled per pixel and the digits stay smooth on the panel's
    colour channels — not a 7-segment or hard bitmap look), drifting through a rainbow, with the
    date and seconds. It adapts to the panel: HH:MM + date + seconds on a tall wall, a one-row
    HH:MM:SS on a 128×32. The face is generated by `tools/genaafont.py` from JetBrains Mono Bold
