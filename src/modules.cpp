...
// how the RS-485 gateway learns what is out on a physical bus, where modules can be added,
...
//
// ORDER MATTERS. The home must go out BEFORE gQuietTime is raised: rs485Send suppresses
...
//
// Safe to call from any task. vmMutex is NEVER held across rs485Send: vbusDeliver re-takes
...
/* ----------------------------------------------------------
   Filesystem
   ----------------------------------------------------------
   The registry is gone, but FATFS is not: /vmods.dat holds the reels' own persisted state
   and /compset.gz holds the companion's settings blob.
---------------------------------------------------------- */
