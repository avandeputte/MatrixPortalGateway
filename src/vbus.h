...
//
// Sits exactly where the UART used to. busSend() frames and sanitizes a
// command as before, then hands the finished bytes to vbusDeliver() instead of
// a UART write. The virtual modules parse them and QUEUE their replies; the bus
// task drains that queue and feeds the bytes back through the same accumulator
// the UART fed, so mqttPublishMsg is reached by the identical path.
...
// ordered and legible in the MQTT mirror.
//
...
// Module think-time before an answer. Non-zero for one reason: busSend is still
...
// Gateway -> modules. Called from busSend while it holds txMutex, so it must
